...
from webbrowser import open as webopen            # Opens URLs in a web browser
...
from groq import Groq                            # Library for querying data from a database
import subprocess                                 # Interact with the system
import requests                                   # Make HTTP requests
import keyboard                                   # Manage keyboard input
...
GroqAPIKey = env_vars.get("GROQ_API_KEY")    # Retrieve the Groq API key
# Define CSS classes for parsing specific elements in HTML content
classes = [
    "cZbUDd", "hbkqGe", "tH0fqk", "srt vk_bk FzvWSb YwPhnf", "pclqee", "tw-Data-text tw-text-small tw-ta",
    "LrzXr", "OSrXXb", "tI6dfc", "vlQcge", "webanswers-webanswers_table__webanswers-table__DoNo ik8A4b gsrt", "sXLaOe",
    "V0h1Ob", "Vrfjeg", "kno-rdesc", "SPZz6b"
]

# Define a user-agent header for making web requests
useragent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.7204.169 Safari/537.36'
...
def GoogleSearchTopic(topic):
    search(topic)       # Uses pywhatkit to perform a Google search.
...
# Asynchronous function to execute commands
async def TranslateAndExecute(commands: list[str]):
    ...
    for command in commands:
        if command.startswith("open "):  # Handle "open" commands.
            ...
            fun = asyncio.to_thread(GoogleSearchTopic, command.removeprefix("google search "))  # Schedule Google search.
            funcs.append(fun)

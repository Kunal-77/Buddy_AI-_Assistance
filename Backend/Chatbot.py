...
def RealtimeInformation():
    ...
    # Extract date and time components using strftime for formatting
    day = current_date_time.strftime("%A")      # Day of the week
    date = current_date_time.strftime("%d")     # Day of the month
    month = current_date_time.strftime("%B")    # Full month name
    year = current_date_time.strftime("%Y")     # 4-digit year
    hour = current_date_time.strftime("%H")     # Hour (24-hour format)
...
def ChatBot(Query):
    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            stop=None  # Specify model to determine when to stop.
        )
